...
public class Hangman {
	...
	public boolean hasFoundTarget(String target, String guess) {
		 ...
		 if(guess.equalsIgnoreCase(target))return true;
		 return false;
	}
	...
	public String getCurrentGuessForDisplay(String currentGuess) {
		...
		String NewGuess1 = currentGuess.toUpperCase().replaceAll("\\*", "_");
		String NewGuess = NewGuess1.replaceAll(".", "$0 ");
		return NewGuess.trim();
	}
	...
	public String updateCurrentGuess(String target, String guess, char letter) {
		...
		StringBuilder anotherguess = new StringBuilder(guess);
		int PossitionOfAttempt = target.indexOf(letter);
		if(targetContains(target, letter)==true)
		{while(PossitionOfAttempt!=-1)
		{
			anotherguess.setCharAt(PossitionOfAttempt, letter);
			PossitionOfAttempt = target.indexOf(letter, PossitionOfAttempt + 1);
		}}
		return anotherguess.toString();
	}
